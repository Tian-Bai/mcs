...
for seed in tqdm(range(beginseed, endseed+1)):
    ...
    cm = conf_model(input_size=cm_dim, hidden_sizes=[32, 32], bn=False).double()
    ...
    for epoch in tqdm(range(T)):
        with torch.no_grad():
            ...
            if family == 2:
                Xcalib_t = torch.tensor(mu.predict(Xcalib_t)).double()
                Xtest_t = torch.tensor(mu.predict(Xtest_t)).double()
            if family == 3:
                Xcalib_t = torch.tensor(np.column_stack((Xcalib_t, mu.predict(Xcalib_t)))).double()
            ...
            if family == 4:
                ...
                Xtest_t = torch.tensor(np.column_stack((Xtest_t, mu.predict(Xtest_t)))).double()
            if family == 5:
                Xcalib_t = torch.tensor(np.column_stack((Xcalib_t, mu.predict(Xcalib_t), Ycalib_t))).double()
                Xtest_t = torch.tensor(np.column_stack((Xtest_t, mu.predict(Xtest_t), mu.predict(Xtest_t)))).double()
        ...
        if epoch % 10 == 0: # validate
            ...
            with torch.no_grad():
                ...
                for itr in range(100):
                    ...
                    if family == 2:
                        Xcalib_v = torch.tensor(mu.predict(Xcalib_v)).double()
                        Xtest_v = torch.tensor(mu.predict(Xtest_v)).double()
                    if family == 3:
                        Xcalib_v = torch.tensor(np.column_stack((Xcalib_v, mu.predict(Xcalib_v)))).double()
                    ...
                    if family == 4:
                        ...
                        Xtest_v = torch.tensor(np.column_stack((Xtest_v, mu.predict(Xtest_v)))).double() # Ytest_v should not be revealed
                    if family == 5:
                        Xcalib_v = torch.tensor(np.column_stack((Xcalib_v, mu.predict(Xcalib_v), Ycalib_v))).double()
                        Xtest_v = torch.tensor(np.column_stack((Xtest_v, mu.predict(Xtest_v), mu.predict(Xtest_v)))).double() # Ytest_v should not be revealed
    ...
    if family == 2:
        Xcalib = torch.tensor(mu.predict(Xcalib)).double()
        Xtest = torch.tensor(mu.predict(Xtest)).double()
    if family == 3:
        Xcalib = torch.tensor(np.column_stack((Xcalib, mu.predict(Xcalib)))).double()
        Xtest = torch.tensor(np.column_stack((Xtest, mu.predict(Xtest)))).double()
    if family == 4:
        Xcalib = torch.tensor(np.column_stack((Xcalib, Ycalib))).double()
        Xtest = torch.tensor(np.column_stack((Xtest, mu.predict(Xtest)))).double() # Ytest should not be revealed
    if family == 5:
        Xcalib = torch.tensor(np.column_stack((Xcalib, mu.predict(Xcalib), Ycalib))).double()
        Xtest = torch.tensor(np.column_stack((Xtest, mu.predict(Xtest), mu.predict(Xtest)))).double() # Ytest should not be revealed
...
all_df.to_csv(os.path.join('results', f"DL_s, setting={setting}, shape={shape}, xdim={x_dim}, ydim={y_dim}, modelid={modelid}, ntest={ntest}, ntrain_f={ntrain_f}, ncalib={ncalib}, seed={beginseed}_{endseed}, family={family}.csv"))